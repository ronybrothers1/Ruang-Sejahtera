    ...
    <div className="min-h-screen bg-neutral-100 text-brand-ink">
      <header className="border-b border-neutral-200 bg-white">
        <div className="member-portal-header-inner mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-5 md:px-6">
          ...
          <div className="flex items-center gap-3"><Link href="/" className="button-secondary">Lihat Website</Link><SessionLogout /></div>
        </div>
      </header>

      <main className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6 md:py-10">
        {query.error === 'forbidden' ? <div className="status-message-error mb-6 rounded-xl border p-4 text-sm font-semibold" role="alert">Akun Anda tidak memiliki akses ke panel administrasi.</div> : null}
        ...
        <section className="mt-9 grid gap-5 md:grid-cols-2 xl:grid-cols-4" aria-label="Fitur akun">
          <Link href="/akun/profil" className="rounded-2xl border border-neutral-200 bg-white p-6 transition hover:-translate-y-1 hover:border-red-200"><UserRound className="text-brand-red" size={22} /><h2 className="mt-5 font-heading text-xl font-extrabold">Profil & keamanan</h2><p className="mt-3 text-sm leading-6 text-neutral-600">Kelola identitas akun dan password.</p></Link>
