...
export default async function ProgramApplicationPage({ searchParams }: { searchParams: Promise<{ submitted?: string; error?: string }> }) {
  ...
  const [applications, programs] = await Promise.all([
    listProgramApplications({ applicantUserId: session.id }),
    listActivePrograms(),
  ]);
  ...
  return (
    <div className="min-h-screen bg-neutral-100 text-brand-ink">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6 md:py-12">
        ...
        {query.error ? <div role="alert" className="status-message-error mt-7 rounded-xl border p-4 text-sm font-semibold">{query.error === 'exists' ? 'Anda sudah memiliki pengajuan aktif untuk program tersebut.' : query.error === 'program' ? 'Program tersebut tidak tersedia.' : 'Pengajuan belum berhasil dikirim. Periksa kembali data dan foto yang diunggah.'}</div> : null}
        {applications.length ? <section className="mt-8 rounded-2xl border border-neutral-200 bg-white p-6 md:p-8" aria-labelledby="application-status-heading"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Status pengajuan</p><h2 id="application-status-heading" className="font-heading text-2xl font-extrabold">Pantau proses pengajuan Anda.</h2></div><span className="count-badge">{applications.length} pengajuan</span></div><div className="mt-6 space-y-4">{applications.map((application) => { const program = programs.find((item) => item.slug === application.programSlug); return <article key={application.id} className="rounded-xl border border-neutral-200 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="font-heading text-xl font-extrabold">{program?.name || application.programSlug}</h3><p className="mt-1 text-sm text-neutral-500">Dikirim {dateLabel(application.createdAt)} · {application.beneficiaryName}</p></div><span className={`inline-flex w-fit rounded-full border px-3 py-1 text-xs font-extrabold ${statusClasses[application.status]}`}>{statusLabels[application.status]}</span></div>{application.reviewNote ? <div className="mt-4 rounded-lg bg-neutral-50 p-4 text-sm leading-6"><strong>Catatan admin:</strong> {application.reviewNote}</div> : null}</article>; })}</div></section> : null}
        <section className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3" aria-label="Pilihan program bantuan">
          {programs.map((program) => <Link key={program.slug} href={`/akun/pengajuan/${program.slug}`} className="group rounded-2xl border border-neutral-200 bg-white p-6 transition hover:-translate-y-1 hover:border-red-300 hover:shadow-lg"><div className="flex items-start justify-between gap-4"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-red-50 text-brand-red"><ClipboardList size={22} aria-hidden="true" /></span><ArrowRight className="text-neutral-400 transition group-hover:translate-x-1 group-hover:text-brand-red" size={20} aria-hidden="true" /></div><p className="mt-6 text-xs font-extrabold uppercase tracking-[.14em] text-brand-red">Program Ruang Sejahtera</p><h2 className="mt-2 font-heading text-2xl font-extrabold">{program.name}</h2><p className="mt-3 text-sm leading-6 text-neutral-600">{program.shortDescription}</p><span className="mt-6 inline-flex text-sm font-bold text-brand-red">Isi pengajuan <ArrowRight size={15} className="ml-2" aria-hidden="true" /></span></Link>)}
        </section>
      </main>
    </div>
  );
