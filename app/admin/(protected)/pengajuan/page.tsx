      ...
      <section className="mt-8 space-y-5" aria-label="Daftar pengajuan program">
        {applications.length ? applications.map((application) => {
          ...
          return <article key={application.id} className="overflow-hidden rounded-2xl border border-neutral-200 bg-white p-6 md:p-8"><div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-3"><p className="text-xs font-extrabold uppercase tracking-[.14em] text-brand-red">{program?.name || application.programSlug}</p><StatusBadge status={application.status === 'approved' ? 'approved' : application.status === 'rejected' ? 'rejected' : application.status === 'revision_required' ? 'revision_required' : application.status === 'under_review' ? 'pending_review' : 'draft'} /></div><h2 className="mt-3 font-heading text-2xl font-extrabold">{application.beneficiaryName}</h2><p className="mt-2 text-sm text-neutral-500">Dikirim {dateLabel(application.createdAt)}</p></div><div className="flex items-center gap-2 text-sm text-neutral-600"><UserRound size={17} aria-hidden="true" /> Identitas calon penerima tersedia</div></div>
            ...
            <div className="mt-6 border-t border-neutral-100 pt-6"><div className="flex items-center gap-2"><ClipboardCheck className="text-brand-red" size={18} aria-hidden="true" /><p className="font-bold">Keputusan review</p></div><form action="/api/admin/program-applications" method="post" className="mt-4 grid gap-4 lg:grid-cols-[220px_1fr_auto] lg:items-end"><input type="hidden" name="id" value={application.id} /><label className="text-sm font-bold">Status<select name="status" defaultValue={application.status === 'submitted' ? 'under_review' : application.status} className="mt-2 min-h-12 w-full rounded-xl border border-neutral-300 bg-white px-4 font-normal"><option value="under_review">Sedang direview</option><option value="revision_required">Minta perbaikan</option><option value="approved">Setujui pengajuan</option><option value="rejected">Tolak pengajuan</option></select></label><label className="text-sm font-bold">Catatan reviewer <span className="font-normal text-neutral-500">(opsional)</span><input name="reviewNote" maxLength={1000} defaultValue={application.reviewNote || ''} placeholder="Tulis alasan atau tindak lanjut" className="mt-2 min-h-12 w-full rounded-xl border border-neutral-300 px-4 font-normal" /></label><button type="submit" className="button-primary">Simpan keputusan</button></form></div>
          </article>;
