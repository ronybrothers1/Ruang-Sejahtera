...
export default async function EditContentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!record || !('category' in record) || !canEditContent(session.role, record.lastEditedBy, session.id)) notFound();
  ...
  return (
    <div className="max-w-4xl">
      ...
      <form action="/api/admin/content" method="post" encType="multipart/form-data" className="mt-8 space-y-6 rounded-2xl border border-neutral-200 bg-white p-6 md:p-8">
        ...
        <label className="block text-sm font-bold">Isi<textarea name="body" required maxLength={20000} rows={14} defaultValue={record.body} className="mt-2 w-full rounded-xl border border-neutral-300 p-4 font-normal leading-7" /></label>
