...
export default async function AdminContentPage({ searchParams }: { searchParams: Promise<{ queued?: string }> }) {
  ...
  return (
    <div>
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="eyebrow">CMS Editorial</p><h1 className="font-heading text-4xl font-extrabold tracking-tight">Konten publik mengikuti workflow, bukan tombol publish instan.</h1><p className="mt-4 max-w-3xl leading-7 text-neutral-600">Registry menyimpan draft, review, published, dan archived. Website publik hanya membaca record berstatus published.</p></div>{canCreate ? <div className="flex flex-wrap gap-2"><Link className="button-secondary" href="/admin/konten/baru/berita"><FilePlus2 size={17} aria-hidden="true" />Berita</Link><Link className="button-secondary" href="/admin/konten/baru/kegiatan"><FilePlus2 size={17} aria-hidden="true" />Kegiatan</Link><Link className="button-secondary" href="/admin/konten/baru/galeri"><FilePlus2 size={17} aria-hidden="true" />Galeri</Link></div> : null}</div>
