...
function WorkflowActions({ item, role, collection }: { item: CollectionItem; role: Parameters<typeof canTransitionPublication>[0]; collection: 'articles' | 'activities' | 'galleries' }) {
  ...
  return <div className="flex flex-col items-start gap-3">{role === 'super_admin' && collection !== 'galleries' ? <Link href={`/admin/konten/edit/${item.id}`} className="button-secondary">Edit</Link> : null}<div className="flex flex-wrap items-start gap-2">{transitions.map((toStatus) => {
    const noteRequired = toStatus === 'revision_required' || toStatus === 'rejected';
    return <form action="/api/admin/content" method="post" key={toStatus} className={noteRequired ? 'w-full rounded-xl border border-neutral-200 bg-neutral-50 p-3' : ''}><input type="hidden" name="intent" value="transition" /><input type="hidden" name="collection" value={collection} /><input type="hidden" name="id" value={item.id} /><input type="hidden" name="toStatus" value={toStatus} />{noteRequired ? <label className="mb-3 block text-xs font-bold">Catatan untuk penulis<input name="reviewNote" required maxLength={1000} placeholder="Jelaskan bagian yang harus diperbaiki atau alasan penolakan" className="mt-2 min-h-11 w-full rounded-lg border border-neutral-300 bg-white px-3 font-normal" /></label> : null}<button type="submit" className={toStatus === 'published' ? 'button-primary' : 'button-secondary'}>{transitionLabel(item.status, toStatus)}</button></form>;
  })}</div></div>;
}
