...
export default async function AdminContentPage({ searchParams }: { searchParams: Promise<{ queued?: string }> }) {
  ...
  return (
    <div>
      ...
      {queued ? <div role="status" className="mt-7 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-900">Perubahan telah diterima backend. Record akan terlihat setelah persistence dan deployment berikutnya selesai.</div> : null}
      {!cms.configured ? <div role="status" className="mt-7 rounded-xl border border-amber-200 bg-amber-50 p-5"><p className="font-bold text-amber-950">Backend tulis CMS masih fail-closed.</p><p className="mt-2 text-sm leading-6 text-amber-900">Form dan workflow UI sudah tersedia, tetapi penyimpanan dinonaktifkan sampai persistence adapter resmi dikonfigurasi. Tidak ada data yang disimpan diam-diam di browser atau filesystem sementara.</p></div> : null}
