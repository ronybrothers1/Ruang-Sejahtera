  ...
  return (
    <div>
      ...
      <section className="mt-8 rounded-2xl border border-neutral-200 bg-white p-6 md:p-8">
        ...
        {reports.length ? <div className="mt-6 space-y-4">{reports.map((report) => <article key={report.id} className="rounded-xl border border-neutral-200 p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-3"><h3 className="font-bold">{report.title}</h3><StatusBadge status={report.status} /></div><p className="mt-1 text-sm text-neutral-500">{report.period} · diperbarui {dateLabel(report.updatedAt)}</p><div className="mt-4 grid gap-3 text-sm sm:grid-cols-4"><span><small className="block text-neutral-500">Penerimaan</small><strong>{formatRupiah(report.totalIncome)}</strong></span><span><small className="block text-neutral-500">Penyaluran</small><strong>{formatRupiah(report.totalDisbursement)}</strong></span><span><small className="block text-neutral-500">Operasional</small><strong>{formatRupiah(report.operationalCost)}</strong></span><span><small className="block text-neutral-500">Saldo</small><strong>{formatRupiah(report.balance)}</strong></span></div></div>{canManage ? <div className="flex flex-wrap gap-2"><Link href={`/admin/transparansi?edit=${report.id}`} className="button-secondary"><Pencil size={15} aria-hidden="true" />Edit</Link>{report.status !== 'published' ? <form action="/api/admin/financial-reports" method="post"><input type="hidden" name="intent" value="publish" /><input type="hidden" name="id" value={report.id} /><button type="submit" className="button-primary"><Send size={15} aria-hidden="true" />Terbitkan</button></form> : <form action="/api/admin/financial-reports" method="post"><input type="hidden" name="intent" value="archive" /><input type="hidden" name="id" value={report.id} /><button type="submit" className="button-secondary"><Archive size={15} aria-hidden="true" />Arsipkan</button></form>}</div> : null}</div></article>)}</div> : <p className="mt-5 text-sm leading-6 text-neutral-500">Belum ada laporan. Buat laporan pertama untuk mengaktifkan integrasi transparansi publik.</p>}
      </section>
    </div>
  );
}
