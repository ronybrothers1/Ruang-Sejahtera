      ...
      <section className="trust-page-section trust-transparency-preview">
        <div className="shell">
          ...
          <div className="trust-transparency-dashboard">
            ...
            <div id="dokumen" className="trust-report-panel"><div><span>Laporan keuangan terbit</span><h2>Riwayat yang dapat diperiksa.</h2></div>{reports.length ? reports.slice(0, 6).map((report) => <div className="trust-report-row" key={report.id}><FileCheck2 size={19} aria-hidden="true" /><span><strong>{report.title}</strong><small>{report.period} · Terbit {report.publishedAt ? new Intl.DateTimeFormat('id-ID', { dateStyle: 'medium' }).format(new Date(report.publishedAt)) : ''}</small></span><span className="text-xs font-bold text-green-700">TERBIT</span></div>) : <p className="mt-4 text-sm leading-6 text-neutral-500">Laporan terbit akan muncul di sini setelah dipublikasikan dari Control Plane.</p>}<Link href="/kebijakan-donasi" className="trust-text-link">Baca kebijakan donasi <ArrowRight size={16} aria-hidden="true" /></Link></div>
          </div>
        </div>
      </section>
    </>
