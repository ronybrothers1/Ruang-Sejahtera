...
- Autentikasi production memakai Clerk, role/status aplikasi tersimpan di Neon PostgreSQL, dan seluruh role menggunakan satu pintu login `/masuk` tanpa kunci akses Super Admin atau approval gate tambahan.
...
Login/register pengguna berada di `/masuk` dan `/daftar`; portal pengguna di `/akun`. Super Admin dapat menambahkan Core Manager dari menu Sistem, sedangkan seluruh role tetap masuk melalui alur Clerk yang sama. Lihat `docs/AUTH-RBAC-PHASE-1.md` dan `docs/ADMIN-CMS.md`.
